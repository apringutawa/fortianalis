...
async def export_pdf(
    ...
    pdf_bytes = _generate_pdf(report, period)
...
async def export_excel(
    ...
    # AI Insight section
    ws1["A12"] = "AI Security Insight"
    ws1["A12"].font = Font(name="Calibri", size=13, bold=True, color="94A3B8")
    ws1["A13"] = f"Analysis: {report.ai_analysis}"
    ws1["A13"].font = Font(name="Calibri", size=10)
    ws1["A13"].alignment = Alignment(wrap_text=True)
    ws1["A14"] = f"Recommendation: {report.ai_recommendation}"
    ws1["A14"].font = Font(name="Calibri", size=10)
    ws1["A14"].alignment = Alignment(wrap_text=True)
    ws1.row_dimensions[13].height = 60
...
async def export_word(
    ...
    # AI Insight
    doc.add_heading("AI Security Insight", level=1)
    doc.add_paragraph().add_run("Analysis: ").bold = True
    doc.paragraphs[-1].add_run(report.ai_analysis or "—")
    doc.add_paragraph().add_run("Recommendation: ").bold = True
    doc.paragraphs[-1].add_run(report.ai_recommendation or "—")
    doc.add_paragraph(f"Powered by: {report.ai_powered_by}").runs[0].font.size = Pt(9)
...
def _generate_pdf(report: Report, period: str) -> bytes:
    try:
    ...
    # ── AI Insight ────────────────────────────────────────────────────────
    elements.append(Paragraph("AI Security Insight", h2_style))
    elements.append(Paragraph(f"<b>Analisis:</b> {report.ai_analysis or '—'}", body_style))
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(f"<b>Rekomendasi:</b> {report.ai_recommendation or '—'}", body_style))
    elements.append(Paragraph(f"<i>Powered by: {report.ai_powered_by or 'Engine'}</i>", small_style))
    elements.append(Spacer(1, 14))
    ...
    if attack_types:
        elements.append(Paragraph("Attack Distribution", h2_style))
        pie_data = [a.get("value", 0) for a in attack_types[:6]]
        pie_colors = [
            colors.HexColor("#06b6d4"), colors.HexColor("#3b82f6"),
        ...
        for i, atk in enumerate(attack_types[:6]):
            drawing3.add(Rect(10, legend_y - i * 18, 12, 12, fillColor=pie_colors[i]))
            drawing3.add(String(28, legend_y - i * 18 + 2, atk.get("name", ""), fontSize=9))
